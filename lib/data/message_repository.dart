...
class MessageRepository {
  ...
  Stream<List<Message>> getMessagesStream(
    ...
    return _firestore
      ...
      return querySnapshot.docs
          .where(
              (documentSnapshot) => isValidDocument(documentSnapshot, userId))
  ...
  Future<void> removeReaction({
    ...
    return await _firestore.doc(path).update(
      {REACTION: _reactionsToMap(builder.build())},
    );
  }
  ...
  static bool isValidDocument(DocumentSnapshot documentSnapshot,
      [String userId = ""]) {
    ...
    switch (docType) {
      ...
      case MessageType.MEDIA:
        return _hasValidAuthor(documentSnapshot) &&
            _isVisibleToUser(documentSnapshot, userId);
        break;
      default:
        return false;
        break;
  ...
  static bool _isVisibleToUser(
      DocumentSnapshot documentSnapshot, String userId) {
    final mediaStatus =
        MediaStatusHelper.valueOf(documentSnapshot[MEDIA_STATUS]);
